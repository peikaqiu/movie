<template>
  <div class="box">
    ...
    <div class="title">
      <p>{{showSet.title}}</p>
      <div class="sunglass">3D眼镜需自备或到影院购买</div>
    </div>
      ...
      <div class="inner-seat-wrapper" ref="innerSeatWrapper">
        <div v-for="row in seatRow">
          ...
          <div v-for="col in seatCol" v-if="seatArray.length>0" class="seat"
            ...
            <!--那个值为-1就不显示 为012就显示自己的样式座位-->
            <div class="inner-seat" @click="handleChooseSeat(row-1,col-1)" v-show="seatArray[row-1][col-1]!==-1"
    ...
    <div class="footer">
      推荐座位
      <div class="btn-buy"><span @click="buySeat">选定</span></div>
      <div class="btn-buy"><span @click="resetSeat">重置</span></div>
      <template v-for="(item,index) in smartChooseMaxNum">
        <div class="btn-buy smart"><span @click="smartChoose(index+1)">{{index + 1}}人</span></div>
      </template>
    </div>
  </div>
</template>
  ...
  export default{
    ...
    methods: {
      ...
      initSeatArray(){
        ...
        this.seatSize = this.$refs.innerSeatWrapper ? parseInt(parseInt(window.getComputedStyle(this.$refs.innerSeatWrapper).width) / this.seatCol) : 0;
        //初始化不是座位的地方
        this.initNonSeatPlace();
        console.log(this.seatArray);
        console.log(this.seatSize);
      },
      initNonSeatPlace: function () { //座位的形状
        for (let i = 0; i < 9; i++) {
